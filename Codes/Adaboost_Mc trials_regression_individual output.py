...
# One-hot encode for AdaBoost (no native categorical support)
...
print("Dataset shape:", X_encoded.shape)
...
for i in range(100):
    X_train, X_test, y_train, y_test = train_test_split(
        X_encoded, y, test_size=0.3, random_state=i
    )

    # --- No of days for failure (BEST HP) ---
    estimator_days = DecisionTreeRegressor(
        max_depth=params_no_days['estimator_max_depth'],
        min_samples_split=params_no_days['estimator_min_samples_split'],
        min_samples_leaf=params_no_days['estimator_min_samples_leaf'],
        random_state=42
    )
    model_days = AdaBoostRegressor(
        n_estimators=params_no_days['n_estimators'],
        learning_rate=params_no_days['learning_rate'],
        estimator=estimator_days,
        random_state=42
    )
    ...
    estimator_pwp = DecisionTreeRegressor(
        max_depth=params_pwp['estimator_max_depth'],
        min_samples_split=params_pwp['estimator_min_samples_split'],
        min_samples_leaf=params_pwp['estimator_min_samples_leaf'],
        random_state=42
    )
    model_pwp = AdaBoostRegressor(
        n_estimators=params_pwp['n_estimators'],
        learning_rate=params_pwp['learning_rate'],
        estimator=estimator_pwp,
        random_state=42
    )
...
print("\nSummary metrics over 100 runs (BEST HYPERPARAMETERS):")
...
output_file = os.path.join(output_dir, 'AdaBoost_regression_metrics_100runs_refined.xlsx')
