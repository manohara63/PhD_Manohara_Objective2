...
for i in range(100):
    ...
    model_days = lgb.LGBMRegressor(**params_no_days)
    model_days.fit(
        X_train, y_train['No of days for failure'],
    ...
    model_pwp = lgb.LGBMRegressor(**params_pwp)
    model_pwp.fit(
        X_train, y_train['PWP at failure'],
        categorical_feature=['Rainfall realisation']
    )
...
output_dir = r'D:\PhD work\Objective 2\FE modelling\ML Modelling\LightGBM'
os.makedirs(output_dir, exist_ok=True)
output_file = os.path.join(output_dir, 'LightGBM_regression_100runs_refined.xlsx')
